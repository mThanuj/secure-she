...
HEIGHT = WIDTH
logging.basicConfig(
    filename=os.path.join("logs", "admin.log"),
    level=logging.INFO,
...
class Application:
    ...
    def getDetails(self):
        ...
        with open(os.path.join("db", "vehicle_database.json"), "r") as f:
            data = json.load(f)
    ...
    def update_map(self):
        while not self.stop_thread:
            ...
            for user in data:
                ...
                icon = folium.Icon(
                    color="red" if data[user]["marked"] else "green",
                    icon="MARKED" if data[user]["marked"] else "",
                )
                folium.Marker(location, icon=icon).add_to(map)

            html = map._repr_html_()

            with open(os.path.join("db", "map.html"), "w") as f:
                f.write(html)
    ...
    def getLocations(self):
        ...
        webbrowser.open_new_tab(os.path.join("db", "map.html"))
    ...
    def activeRequests(self):
        ...
        with open(os.path.join("db", "pending_requests.json"), "r") as f:
            data = json.load(f)
        ...
        for id, vehicle in pending_requests.items():
            button = tk.CTkButton(
                ...
                text=f"Resolve {id}",
                font=("Arial", 15),
                command=lambda request_id=id, vehicle=vehicle: self.resolveRequest(
                    request_id, vehicle
                ),
            )
            button.pack(pady=10)
    ...
    def resolveRequest(self, request_id, vehicle):
        ...
        with open(os.path.join("db", "vehicle_database.json"), "w") as f:
            json.dump(data, f, indent=4)
    ...
    def getLog(self):
        ...
        with open(os.path.join("logs", "admin.log")) as f:
            data = f.read()
