...
class Application:
    def __init__(self):
        ...
        self.removeTrustedContacts = tk.CTkButton(
            self.root,
            text="Remove Trusted Contacts",
            width=WIDTH,
            height=HEIGHT,
            command=self.removeContacts,
        )
        self.btn4 = tk.CTkButton(self.root, text="Btn 4", width=WIDTH, height=HEIGHT)
    ...
    def sendMyLocation(self):
        ...
        with open(os.path.join("db", "pending_requests.json")) as f:
            data = json.load(f)
        with open(os.path.join("db", "vehicle_database.json")) as f1:
        ...
        with open(os.path.join("db", "vehicle_database.json"), "w") as f1:
            json.dump(vehicle_data, f1, indent=4)

        with open(os.path.join("db", "pending_requests.json"), "w") as f:
    ...
    def addContacts(self):
        ...
        self.addButton = tk.CTkButton(
            self.newWindow, text="Add", font=("Arial", 15), command=self.addContact
        )
        self.addButton.pack(pady=10)

        self.closeButton = tk.CTkButton(
            self.newWindow,
            text="Close",
            font=("Arial", 15),
            command=self.closeNewWindow,
        )
    ...
    def addContact(self):
        ...
        if not contact.isdigit():
            self.confirmationLabel.configure(text="Invalid format")
            return

        if (len(contact) == 12 and contact[0:3] != "+91") or len(contact) != 10:
            self.confirmationLabel.configure(text="Invalid length")
            return
    ...
    def removeContacts(self):
        ...
        self.removeButton = tk.CTkButton(
            self.newWindow,
            text="Remove",
            font=("Arial", 15),
